...
CAP_WIDTH       = 640 #640                   #出力動画の幅
CAP_HEIGHT      = 480 #480                   #出力動画の高さ
...
class AbstractModel(ABC):
    ...
    _runtime: str = 'onnx'
    _model_path: str = ''
    _obj_class_score_th: float = 0.20
    _attr_class_score_th: float = 0.40
...
class YOLOv9(AbstractModel):
    ...
    def _preprocess(
        ...
        """
        # 例：モデルが期待する入力サイズが1x3x128x160の場合
        input_height = self._input_shapes[0][2]  # 128
        input_width = self._input_shapes[0][3]   # 160
...
#        )
    ...
    while True:
        ...
        for box in boxes:
            print(box)
            print(((box.x1 + box.x2) / 2) / CAP_WIDTH)
